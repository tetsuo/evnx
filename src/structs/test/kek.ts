...
test("array ops", t => {
    ...
    fixtures.forEach((f, ix) => {
        ...
        s.on("data", (patches: any) => {
            allPatches.push(patches)
        })

        s.on("end", () => {
            process.nextTick(() => {
                allPatches.forEach((patch: any) => {
...
test("apply patches", t => {
    ...
    const m1 = new Kek<IFooKek>([ x ])
    const m2 = new Kek<IFooKek>([ { name: "foo" } ])
